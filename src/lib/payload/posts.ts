...
export function mapPayloadPost(doc: PayloadPostDoc): Post {
  const published = doc.publishedAt ? parseISO(doc.publishedAt) : null;
  const hasDate = published !== null && isValid(published);
  ...
  return {
    ...
    label: doc.label,
    date: hasDate ? format(published, "MMM d, yyyy") : "",
    dateISO: hasDate ? format(published, "yyyy-MM-dd") : "",
    read: doc.readTime,
    title: doc.title,
    excerpt: doc.excerpt,
