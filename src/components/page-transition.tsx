...
export function PageTransitionCurtain() {
  ...
  // Idempotent: only reveals once from the "covered" state, so it is safe to
  // call from both the navigate path and the router-status effect below.
  ...
  useEffect(() => {
    _triggerFn = async (to: string) => {
      ...
      // Fast/cached navigations may not emit a routerStatus change for the
      // effect below to observe — reveal directly once navigation resolves.
      startReveal();
    };
